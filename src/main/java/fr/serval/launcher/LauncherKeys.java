package fr.serval.launcher;

public class LauncherKeys {
    public static String LAUNCHER_NAME = "Lancer SERVAL";

    public static String USER_HOME_DIR = "user.home";

    public static String START = "Démarrer";
    public static String SAVE = "Sauvegarder";
    public static String CANCEL = "Annuler";
    public static String CLOSE = "Fermer";
    public static String MANAGE_PLUGINS = "Gérer les plugins";

    public static String MANAGER_NAME = "Gestionnaire de plugins";

    public static String ERROR_PLUGIN_IMPORTER = "Erreur d'import des plugins";
    public static String ERROR_NO_HOME_DIR = "Pas de répertoire utilisateur trouvé";
    public static String ERROR_CAN_NOT_CREATE = "Impossible de créer";

    public static String PLUGIN_STATE_NOMINAL = "Fichier binaire trouvé";
    public static String PLUGIN_STATE_NOT_FOUND = "Fichier binaire introuvable";
    public static String ERROR_NO_PLUGIN_FOUND = "Pas de plugin trouvés dans le fichier de configuration";
}
